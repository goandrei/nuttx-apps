...
#define REMOTE_HOST "192.168.0.162"
...
static int client_ssl_init(struct client *c) {
    ...
    int ret = wolfSSL_set_alpn_protos(c->ssl, (const unsigned char *)ALPN, sizeof(ALPN) - 1);
    if(ret != WOLFSSL_SUCCESS) {
        fprintf(stderr, "wolfSSL_set_alpn_protos failed : %d\n", ret);
        //return -1;
    }
    if (!numeric_host(REMOTE_HOST)) {
        wolfSSL_set_tlsext_host_name(c->ssl, REMOTE_HOST);
    }
...
static int client_init(struct client *c) {
    ...
    host.sin_addr.s_addr = inet_addr("192.168.0.162");
    memset(host.sin_zero, 0, sizeof(host.sin_zero));

    remote_addr = (struct sockaddr*)&host;
